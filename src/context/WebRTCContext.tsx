...
export const WebRTCProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const setupMediaStream = async () => {
    try {
      ...
      try {
        ...
        console.warn(
          "Could not get both audio and video, trying audio only:",
          initialErr
        );

        // Add a more user-friendly message for device in use errors
        if (initialErr instanceof Error && 
            initialErr.name === "NotReadableError" && 
            initialErr.message.includes("Device in use")) {
          showToast.warning("Camera or microphone is already in use by another application. Please close other applications using your camera/mic.");
        }
        ...
        try {
          ...
          try {
            ...
            console.warn(
              "Could not get any media devices, creating empty stream:",
              videoErr
            );
  ...
  // Inside the createPeerConnection function
  const createPeerConnection = (participantId: string) => {
    ...
    if (peerConnections.current[participantId]) {
      ...
      if (
        ...
        console.log(
          `Existing connection for ${participantId} is in bad state (${existingPc.connectionState}/${existingPc.iceConnectionState}), recreating`
        );
        existingPc.close();
    ...
    peerConnection.onconnectionstatechange = () => {
      ...
      if (
        ...
        console.warn(
          `Connection to ${participantId} ${peerConnection.connectionState}`
        );
      }
    };
    ...
    peerConnection.oniceconnectionstatechange = () => {
      console.log(
        `ICE connection state for ${participantId}: ${peerConnection.iceConnectionState}`
      );
    };
        ...
        if (authState.user && meetingId) {
          await setDoc(
            doc(
              ...
              meetingId,
              "answers",
              `${authState.user.uid}_to_${fromId}_${Date.now()}`
            ),
            {
              answer: {
                type: answer.type,
  ...
  const handleAnswer = async (
    ...
    try {
      ...
      console.log(
        `Handling answer from ${fromId}, current state: ${pc.signalingState}`
      );
      ...
      if (pc.signalingState === "have-local-offer") {
        ...
        console.log(
          `Connection for ${fromId} is in have-remote-offer state, cannot process answer directly`
        );
        ...
        try {
          ...
          console.log(
            `Recreating connection for ${fromId}, shouldInitiate: ${shouldInitiate}`
          );
        ...
        console.log(
          `Connection for ${fromId} is already in stable state, ignoring answer`
        );
      } else {
        console.warn(
          `Cannot set remote answer in state: ${pc.signalingState} for ${fromId}`
        );

        // If we're in a bad state, reset the connection
        console.log(
          `Resetting connection for ${fromId} due to signaling state conflict`
        );
        pc.close();
        delete peerConnections.current[fromId];
              ...
              if (authState.user) {
                await setDoc(
                  doc(
                    ...
                    meetingId,
                    "offers",
                    `${authState.user.uid}_to_${participantId}`
                  ),
                  {
                    offer: {
                      type: offer.type,
  ...
  const toggleAudio = () => {
    if (localStream && authState.user) {
      ...
      setParticipants((prev) =>
        prev.map((p) =>
          p.id === authState.user?.uid ? { ...p, audioEnabled: newState } : p
        )
      );
    }
  };
  ...
  const toggleVideo = () => {
    if (localStream && authState.user) {
      ...
      setParticipants((prev: Participant[]) =>
        prev.map((p) =>
          p.id === authState.user?.uid ? { ...p, videoEnabled: newState } : p
        )
      );
    }
  };
